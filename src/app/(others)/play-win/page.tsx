...
export default function PlayWinRewards() {
  ...
  return (
    <div className="bg-surface-bright text-on-surface min-h-screen py-8 max-w-7xl mx-auto px-6 space-y-12">
      ...
      <section className="relative py-8 flex flex-col lg:flex-row items-center gap-12 overflow-hidden">
        
        {/* Left Content */}
        <div className="w-full lg:w-1/2 space-y-6">
          <div className="inline-flex items-center gap-1.5 bg-orange-100 text-orange-600 px-4 py-1.5 rounded-full text-xs font-bold shadow-sm">
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <button 
              onClick={handleSpin}
              disabled={spinning}
              className="bg-orange-500 hover:bg-orange-600 disabled:bg-gray-400 text-white px-8 py-3.5 rounded-full font-bold text-xs shadow-md shadow-orange-500/10 active:scale-95 transition-all"
            >
              {spinning ? "Spinning..." : "Play Now"}
            </button>
            <button className="border-2 border-gray-300 text-gray-700 px-8 py-3.5 rounded-full font-bold text-xs hover:bg-gray-50 transition active:scale-95">
              View Leaderboard
            </button>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-center justify-center relative">
          ...
          {/* Wheel Frame */}
          <div className="relative w-64 h-64 md:w-80 md:h-80 rounded-full border-[10px] border-neutral-900 shadow-2xl overflow-hidden flex items-center justify-center bg-neutral-900">
            ...
            >
              {/* Conic sections via pure conic gradient */}
              <div 
                className="w-full h-full rounded-full"
                style={{ 
                  background: 'conic-gradient(from 0deg, #ff6900 0deg 60deg, #ffba20 60deg 120deg, #56657d 120deg 180deg, #a14000 180deg 240deg, #7c5800 240deg 300deg, #d3e3ff 300deg 360deg)' 
                }} 
              />
            ...
            <div className="w-16 h-16 rounded-full bg-white shadow-xl z-20 flex items-center justify-center border border-gray-150 cursor-pointer" onClick={handleSpin}>
              <span className="text-lg font-bold text-orange-500 select-none">SPIN</span>
            </div>
          </div>

          {/* Needle Indicator */}
          <div className="absolute top-[-8px] z-30">
            <div className="w-6 h-8 bg-red-500 rounded-b-full shadow-md" />
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-6 pt-6">
        ...
        <div className="col-span-1 md:col-span-8 bg-white border border-gray-100 rounded-3xl p-8 relative overflow-hidden flex flex-col justify-between min-h-[300px] shadow-sm">
          <div className="space-y-3 relative z-10 max-w-sm">
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              The gravity-defying challenge. Watch the golden spheres cascade and multiply your points.
            </p>
            <div className="pt-2 text-orange-500 font-extrabold text-lg">
          ...
          <div className="pt-4 flex items-center gap-4 relative z-10">
            <button 
              ...
              className="bg-orange-500 hover:bg-orange-600 disabled:bg-gray-400 text-white px-6 py-2 rounded-full font-bold text-xs transition active:scale-95 shadow-md shadow-orange-500/10"
            >
              {ballDropping ? "Dropping..." : "Drop Ball"}
            </button>
            {ballMultiplier && (
              <span className="text-amber-600 bg-amber-50 border border-amber-100 px-3 py-1 rounded-full text-xs font-bold animate-bounce">
          ...
          <div className="absolute right-6 bottom-6 top-6 w-1/3 opacity-20 pointer-events-none flex flex-col justify-around items-center">
            ...
            <div className="w-6 h-6 rounded-full border border-orange-500 bg-orange-500/20 animate-ping" />
          </div>
        </div>

        {/* Scratch Cards */}
        <div className="col-span-1 md:col-span-4 bg-white border border-gray-100 rounded-3xl p-8 flex flex-col justify-between min-h-[300px] shadow-sm group">
          <div className="space-y-3">
            <div className="w-10 h-10 bg-orange-500/10 text-orange-600 rounded-full flex items-center justify-center">
              <span className="material-symbols-outlined text-lg">auto_awesome</span>
            </div>
            <h2 className="text-xl font-bold text-gray-800">Scratch Cards</h2>
            <p className="text-xs text-gray-500 leading-relaxed">
          ...
          <div 
            ...
            className="h-28 bg-gray-50 hover:bg-orange-50 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center cursor-pointer transition-colors relative"
          >
            {scratchRevealed ? (
              <div className="text-center space-y-2">
                ...
                <p className="text-[10px] text-gray-500 font-bold">You Won 1,000 pts!</p>
                <button 
                  onClick={(e) => { e.stopPropagation(); handleResetScratch(); }}
                  className="text-[9px] text-orange-500 underline uppercase hover:text-orange-600"
                >
                  Reset Card
                </button>
              </div>
        ...
        {/* Competitions */}
        <div className="col-span-1 md:col-span-5 bg-amber-500/10 border border-amber-500/20 rounded-3xl p-8 flex flex-col justify-between min-h-[300px]">
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-amber-900">Competitions</h2>
            <div className="space-y-2">
              {competitions.map((comp, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-white/70 backdrop-blur-sm rounded-xl border border-white/30">
                  ...
                  <span className="text-[10px] font-bold text-amber-950 uppercase tracking-wider">{comp.timeLeft}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="pt-6 space-y-1">
            ...
            <p className="text-[10px] font-bold text-amber-900 uppercase tracking-widest">Global Prize Pool</p>
          </div>
        </div>

        {/* Daily Challenges */}
        <div className="col-span-1 md:col-span-7 bg-white border border-gray-100 rounded-3xl p-8 flex flex-col justify-between min-h-[300px] shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xl font-bold text-gray-800">Daily Challenges</h2>
              <p className="text-xs text-gray-500 mt-0.5">Complete tasks to unlock bonus spins.</p>
            </div>
            ...
            {challenges.map((challenge) => (
              <div key={challenge.id} className="space-y-1.5">
                <div className="flex justify-between text-xs font-bold text-gray-700">
                  ...
                  <span className="text-orange-500">{challenge.reward}</span>
                </div>
