      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[90vh] px-4 text-white overflow-hidden">
        ...
        {!method ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl">
            ...
            <Card
              onClick={() => !isAdmin && setMethod("number")}
              className={`bg-white/90 text-gray-800 rounded-2xl shadow-xl transition-transform ${isAdmin ? 'opacity-70 cursor-not-allowed' : 'hover:scale-105 cursor-pointer'}`}
            >
              <CardContent className="flex flex-col items-center justify-center p-6 text-center">
          ...
          >
            {method === "qr" && (
              <>
                <h2 className="text-2xl font-bold mb-4 text-orange-600">{isAdmin ? 'Participant QR Code' : 'Your QR Code'}</h2>
                <div className="bg-white p-4 rounded-xl inline-block">
                  <QRCode value={participantId} size={180} />
                </div>
                <p className="text-sm text-gray-600 mt-4">
                  {isAdmin ? 'This is the participant unique QR Code.' : 'Show this QR code at the counter to redeem your rewards.'}
                </p>
              </>
            )}

            {method === "number" && !isAdmin && (
              <>
