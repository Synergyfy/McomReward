...
import { useGetCreditsBalance, useGetCreditsRules, useGetCreditsHistory, useUnlockCredits } from '@/services/cashback/hook';
...
export default function CreditsPage() {
    ...
    const { mutate: unlockCredits, isPending: isUnlocking } = useUnlockCredits();

    const [isUnlockModalOpen, setIsUnlockModalOpen] = useState(false);
    const [selectedLevelId, setSelectedLevelId] = useState<number | null>(null);

    const credits = balanceData?.credits ?? 0;
    const balance = balanceData?.availableCashback ?? 0;
    const currentLevel = balanceData?.progression?.currentLevel ?? 0;

    const levels = balanceData?.progression?.allLevels ?? [];
    const nextLevel = balanceData?.progression?.nextLevel ?? levels[levels.length - 1];
    ...
    const handleConfirmUnlock = () => {
        ...
        if (balance < selectedLevel.matchingContribution) {
        ...
        unlockCredits(selectedLevel.level, {
            onSuccess: (newBalance) => {
                setIsUnlockModalOpen(false);
                toast.success("Reward Unlocked!", {
                    description: `Level ${selectedLevel.level} activated. Your wallet has been credited.`,
                        ...
                        <div className="flex flex-wrap justify-center md:justify-start gap-4">
                            <div className="px-6 py-3 bg-[#101415] backdrop-blur-md rounded-2xl border border-white/5 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-semibold text-white">£{balance.toFixed(2)}</p>
                                </div>
                            </div>
                                        ...
                                        >
                                            <div className="space-y-2">
                                                ...
                                                <ul className="text-xs space-y-1.5 list-disc pl-4 text-gray-300">
                                                    ...
                                                    <li>Platform will match with <span className="text-white font-semibold">£{(lv.totalCashback - lv.matchingContribution)}</span></li>
            ...
            <Dialog open={isUnlockModalOpen} onOpenChange={setIsUnlockModalOpen}>
                <DialogContent className="sm:max-w-md bg-[#1d2022] border border-white/5 rounded-[2rem] p-0 overflow-hidden shadow-2xl text-white">
                    ...
                    <div className="p-8 space-y-6">
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-4 bg-blue-500/10 rounded-2xl border border-blue-500/20">
                                    ...
                                    <p className="text-xl font-bold text-white">£{((selectedLevel?.totalCashback ?? 0) - (selectedLevel?.matchingContribution ?? 0))}</p>
                                </div>
                            </div>
                        ...
                        {balance < (selectedLevel?.matchingContribution || 0) && (
                            <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] text-red-300 leading-tight mt-1">
                                        You need to top up <span className="font-bold">£{((selectedLevel?.matchingContribution || 0) - balance).toFixed(2)}</span> to unlock this reward.
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="space-y-3">
                            <Button 
                                ...
                                disabled={balance < (selectedLevel?.matchingContribution || 0)}
                            ...
                            >
                                {balance < (selectedLevel?.matchingContribution || 0) 
                                    ? `Top Up £${((selectedLevel?.matchingContribution || 0) - balance).toFixed(2)} to Unlock` 
