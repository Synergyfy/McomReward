...
export default function ClaimCampaignModal({
  ...
  const [isSelectRewardModalOpen, setIsSelectRewardModalOpen] = useState(false);
  ...
  const handleProceedFromRewardSelection = (selectedRewardIds: string[]) => {
    if (selectedCampaignId) {
      const queryParams = new URLSearchParams({
        business_reward_ids: JSON.stringify(selectedRewardIds),
      }).toString();

      router.push(`/dashboard/campaigns/preview/${selectedCampaignId}?${queryParams}`);
    }
  };
  ...
  return (
    ...
    onProceed={handleProceedFromRewardSelection}
  />
  </>
  );
}