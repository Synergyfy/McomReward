...
export default function MyCampaignsContent({ data, isLoading }: MyCampaignsContentProps) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-8 bg-[#f9fafb] text-gray-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map((i) => (
            <Card key={i} className="overflow-hidden shadow-lg rounded-2xl bg-white border border-gray-200">
  ...
  return (
    <div className="space-y-8 bg-[#f9fafb] text-gray-800">
      <div className="text-center">
        <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight">My Campaigns</h1>
        <p className="mt-4 text-lg text-gray-500">Track your progress and see the rewards you&apos;re working towards.</p>
      </div>
      ...
      {campaigns.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-gray-200">
          <div className="max-w-md mx-auto px-4">
            ...
            <Link href="/campaigns">
              <Button size="lg" className="bg-[#f54900] hover:bg-[#f54900]/95 text-white font-bold py-3 px-8 rounded-full shadow-lg transform transition hover:scale-105 border-none">
                Start Earning
              </Button>
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {campaigns.map((campaign: MyCampaign) => {
            ...
            return (
              <Card
                key={campaign.id}
                className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-2xl cursor-pointer bg-white border border-gray-200"
              ...
              >
                <div className="relative h-40 w-full bg-gray-50">
                  {campaign.bannerUrl ? (
                    ...
                    <div className="flex items-center justify-center h-full text-gray-405">No Image</div>
                  )}
                ...
                <CardContent className="p-6 text-gray-800">
                  <h2 className="text-xl font-bold mb-2 text-gray-900">{campaign.name}</h2>
                  <p className="text-gray-500 mb-4 text-sm line-clamp-2">{campaign.campaignMessage || 'No description available.'}</p>

                  <div className="flex justify-between text-sm mb-1">
                    <span className="font-semibold text-gray-700">Balance: {campaign.balance}</span>
                    {campaign.regularPointsThreshold && (
                      <span className="text-gray-400">Target: {campaign.regularPointsThreshold}</span>
                    )}
                  </div>
                  ...
                  {campaign.regularPointsThreshold ? (
                    <>
                      ...
                      <p className="text-xs text-right text-gray-400 mt-1">{progress.toFixed(0)}% to next reward</p>
                    </>
                  ) : (
