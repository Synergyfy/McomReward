...
interface StampRewardsContentProps {
    // Union type to handle both Admin (PaginatedResponse) and Customer (GetConsumerStampCardsResponse) structures if they differ
...
export default function StampRewardsContent({
    ...
    const handleRedeem = (card: ConsumerStampCard) => {
        if (isAdmin) return; // Disable redeem for admin
        setSelectedCard(card);
    ...
    const handleStartCard = (reward: DiscoverableStampReward) => {
        if (isAdmin) return; // Disable start for admin
        if (startCard) {
                                    ...
                                    <Card key={reward.id} className="group overflow-hidden border border-gray-100 bg-white dark:bg-gray-900 hover:shadow-xl hover:shadow-orange-500/10 transition-all duration-300 hover:-translate-y-1">
                                        ...
                                        <CardContent className="p-5">
                                            ...
                                            <Button
                                                ...
                                                className="w-full gap-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:opacity-50"
                                            >
