...
import { PlusCircle, Pencil, ChevronLeft, ChevronRight, MoreHorizontal, Copy } from 'lucide-react'; // Added Copy icon
...
export default function CampaignsListPage() {
  ...
  const { data: createdCampaignsData, isLoading: isLoadingCreated } = useGetMyCreatedCampaigns(createdPage, limit, businessId);
  const { data: claimedCampaignsData, isLoading: isLoadingClaimed } = useGetMyClaimedCampaigns(claimedPage, limit, businessId);

  // Admin should not be able to modify or create campaigns when impersonating
  const isImpersonating = true;

  const handleCopyLink = (campaignId: string) => {
    const campaignUrl = `${window.location.origin}/campaigns/${campaignId}`; // This might need to be specific to the public view
  ...
  const handleCreateFromScratch = useCallback(() => {
    // Disabled in impersonation view
    toast.error('Creating campaigns is disabled in admin preview mode.');
  }, []);

  const handleSelectTemplate = useCallback((template: CampaignTemplate) => {
    // Disabled in impersonation view
    toast.error('Creating campaigns from templates is disabled in admin preview mode.');
  }, []);
          ...
          >
            ...
            <CardContent className="pt-16 p-5 text-center">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  <Link href={`/admin/view-business/${businessId}/campaigns/${campaign.id}`}>View Campaign</Link>
                </Button>
                <Button
                  asChild
                  variant="outline"
                ...
                >
                  <Link href={`/admin/view-business/${businessId}/campaigns/edit/${campaign.id}`}>
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <Button onClick={() => setIsClaimModalOpen(true)} disabled={isImpersonating}>
