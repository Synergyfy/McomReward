...
export default function BusinessProfilePage() {
  ...
  // Admin should not be able to edit when impersonating
  const [editing, setEditing] = useState(false); // Always false for admin view
  ...
  const { data: profile, isLoading: isLoadingProfile, isError: isErrorProfile } = useGetBusinessProfile(businessId);
  const { data: subscription, isLoading: isLoadingSubscription, isError: isErrorSubscription } = useGetMySubscription(businessId);
  const { mutate: updateProfile, isPending: isUpdating } = useUpdateBusinessProfile(businessId);
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-sm  mt-8 p-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between border-b border-gray-100 pb-6 mb-8">
        <div className="flex items-center gap-6">
          ...
          <div className="relative">
            <Image
              ...
              height={96}
            />
            {editing && ( // Show camera icon only if editing is enabled (it's not for admin view)
              <>
                <input
                  type="file"
                  ref={logoInputRef}
        ...
        {/* Edit / Save Button */}
        {/* Disable editing button when impersonating */}
        <button
          onClick={() => (editing ? handleSave() : setEditing(true))}
          disabled={isUpdating || true} // Always disable for admin impersonation view
        ...
        >
          {isUpdating ? 'Saving...' : editing ? 'Save Changes' : 'View Profile'} {/* Changed text to View Profile */}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input
            ...
            disabled={!editing || true} // Always disable for admin impersonation view
